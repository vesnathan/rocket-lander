...
import { RocketSprite, createRocketWithPlaceholder } from '../sprites/RocketSprite';
...
export class GameScene extends BaseScene {
    ...
    this.tweens.add({
      ...
      onUpdate: () => {
        ...
        for (let i = 0; i < tail.length; i++) {
          ...
          if (segment) {
            segment.setPosition(prevX - dx * (i + 1), prevY - dy * (i + 1));
          }
        }
      },
      onComplete: () => {
  ...
  private createLaser(hazard: {
    ...
    const container = renderLaserField(this, {
      ...(hazard.emitterX !== undefined && { emitterX: hazard.emitterX }),
      ...(hazard.emitterY !== undefined && { emitterY: hazard.emitterY }),
      ...(hazard.receiverX !== undefined && { receiverX: hazard.receiverX }),
      ...(hazard.receiverY !== undefined && { receiverY: hazard.receiverY }),
      ...(hazard.length !== undefined && { length: hazard.length }),
      ...(hazard.angle !== undefined && { angle: hazard.angle }),
      ...(hazard.width !== undefined && { width: hazard.width }),
    });
  ...
  private updateWarpZones(delta: number): void {
    ...
    for (const [_id, zone] of this.warpZones) {
  ...
  private showLevelCompleteOverlay(
    ...
    if (hasAnyHighScores) {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        fakeLevelScores.push({ score: lvlScore, time: lvlTime, name: fakeNames[nameIndex] ?? 'AAA' });
      }
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        fakeGameScores.push({ score: gameScore, name: fakeNames[nameIndex] ?? 'AAA' });
      }
