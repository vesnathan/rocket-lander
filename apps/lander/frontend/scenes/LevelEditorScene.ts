...
export class LevelEditorScene extends Phaser.Scene {
  ...
  private createPaletteIcon(type: EditorEntityType, size: number): Phaser.GameObjects.Graphics | Phaser.GameObjects.Container {
    ...
    switch (type) {
      ...
      case 'multiplierBubble': {
        ...
        g.beginPath();
        const first = hexPoints[0];
        if (first) {
          g.moveTo(first.x * 1.2, first.y * 1.2);
        }
        for (let i = 1; i < 6; i++) {
          const pt = hexPoints[i];
          if (pt) g.lineTo(pt.x * 1.2, pt.y * 1.2);
        }
        g.closePath();
        g.fillPath();
        // Main hexagon
  ...
  private createEntityAt(type: EditorEntityType, x: number, y: number): void {
    ...
    } else if (entity.entityType === 'patrolShip' || entity.entityType === 'warpZone' || entity.entityType === 'gravityWell' ||
               entity.entityType === 'laserField' || entity.entityType === 'bubbleGun') {
  ...
  private updatePropertiesPanel(): void {
    ...
    if (entity.entityType === 'multiplierBubble') {
    ...
    } else if (entity.entityType === 'landingPad') {
      ...
      this.propsControls.push(btn, btnText);
    } else if (entity.entityType === 'patrolShip') {
    ...
    } else if (entity.entityType === 'laserField') {
      const hazard = entity as EditorHazard;
      const data = hazard.data as any;
    ...
    } else if (entity.entityType === 'terrain') {
  ...
  private refreshSpawnPointVisual(): void {
    ...
    const spawnEntity = {
      editorId: 'spawn-point',
      entityType: 'spawnPoint' as const,
      x: state.spawnPoint.x,
      y: state.spawnPoint.y,
      bubbleColor: state.spawnPoint.bubbleColor ?? 'green',
    };
  ...
  private showLevelManager(): void {
    ...
    // Handle drag end
    const onPointerUp = (_pointer: Phaser.Input.Pointer) => {
