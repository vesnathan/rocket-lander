...
export class EnemyAI {
  ...
  constructor(
    ...
    if (
      ...
      this.config.patrolPath.length > 0
    ) {
      this.position = { ...this.config.patrolPath[0].position };
  ...
  private transitionToReturn(): void {
    ...
    for (let i = 0; i < this.config.patrolPath.length; i++) {
      const distance = vec2Magnitude(
        vec2Sub(this.config.patrolPath[i].position, this.position)
      );
  ...
  reset(): void {
    ...
    if (this.config.patrolPath.length > 0) {
      this.position = { ...this.config.patrolPath[0].position };
    }
