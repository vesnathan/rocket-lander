...
export function createDefaultEditorState(): EditorState {
  return {
    ...
    terrain: [
      // Default walls
      { editorId: generateEditorId(), type: 'terrain', x: 0, y: 0, width: 5, height: 356 },
      { editorId: generateEditorId(), type: 'terrain', x: 635, y: 0, width: 5, height: 356 },
      { editorId: generateEditorId(), type: 'terrain', x: 5, y: 0, width: 630, height: 5 },
      { editorId: generateEditorId(), type: 'terrain', x: 5, y: 351, width: 630, height: 5 },
    ],
    landingPads: [
      {
...
export function levelToEditorState(level: Level): EditorState {
  return {
    ...
    landingPads: level.landingPads.map((p) => ({
      ...p,
      editorId: generateEditorId(),
      type: 'landingPad' as const,
    })),

    fuelPickups: (level.fuelPickups ?? []).map((f) => ({
      ...f,
      editorId: generateEditorId(),
      type: 'fuelPickup' as const,
    })),
    ...
    multiplierBubbles: (level.multiplierBubbles ?? []).map((m) => ({
      ...m,
      editorId: generateEditorId(),
      type: 'multiplierBubble' as const,
    })),
    ...
    iceLayers: (level.iceLayers ?? []).map((i) => ({
      ...i,
      editorId: generateEditorId(),
      type: 'iceLayer' as const,
    })),

    hazards: (level.hazards ?? []).map((h) => ({
      editorId: generateEditorId(),
      type: h.type as EditorHazard['type'],
      data: h,
    })),
...
export function editorStateToLevel(state: EditorState): Level {
  return {
    ...
    terrain: state.terrain.map(({ editorId, type, ...rest }) => rest),

    landingPads: state.landingPads.map(({ editorId, type, ...rest }) => rest),

    fuelPickups: state.fuelPickups.length > 0
      ? state.fuelPickups.map(({ editorId, type, ...rest }) => rest)
      : undefined,

    collectibles: state.collectibles.length > 0
      ? state.collectibles.map(({ editorId, type, ...rest }) => rest)
      : undefined,

    multiplierBubbles: state.multiplierBubbles.length > 0
      ? state.multiplierBubbles.map(({ editorId, type, ...rest }) => rest)
      : undefined,

    checkpoints: state.checkpoints.length > 0
      ? state.checkpoints.map(({ editorId, type, ...rest }) => rest)
      : undefined,

    iceLayers: state.iceLayers.length > 0
      ? state.iceLayers.map(({ editorId, type, ...rest }) => rest)
      : undefined,

    hazards: state.hazards.length > 0
      ? state.hazards.map((h) => h.data)
      : undefined,

    enemies: state.enemies.length > 0
      ? state.enemies.map((e) => e.data)
      : undefined,
...
export class EditorStateManager {
  ...
  undo(): EditorState | null {
    ...
    this.historyIndex--;
    this.currentState = cloneEditorState(this.history[this.historyIndex]);
    return this.currentState;
  }
  ...
  updateEntityPosition(editorId: string, x: number, y: number, quiet = false): void {
    ...
    if (entity) {
      ...
      if (entity.type === 'patrolShip' || entity.type === 'laserField' ||
          ...
          entity.type === 'gravityWell' || entity.type === 'enemy') {
  ...
  updateEntitySize(editorId: string, width: number, height: number, quiet = false): void {
    ...
    if (entity) {
      ...
      if (entity.type === 'patrolShip' || entity.type === 'warpZone') {
